...
def run_with_tools(role: str, goal: str, vault_env: dict | None = None,
    ...
    for step in range(max_steps):
        user_msg = (
            ...
            '  {"thought": "...", "tool": "<name>", "args": {...}}  to call a tool, or\n'
        ...
        if not action:
            ...
            transcript.append(("system", "ERROR: no valid JSON action found; respond again."))
            continue
    ...
    summary = chat(
        role,
        "You hit the step budget. Summarise the result for the human in 6 lines max.\n\n"
        f"Transcript:\n{_format_transcript(transcript)}",
        vault_env=vault_env,
    ).text
    return summary
